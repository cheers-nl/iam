...
This pattern is not hypothetical. Team Vault is the AWS-native re-implementation of the GroupSecret feature in **Stellen** ([stellenapp.com](https://stellenapp.com/)) — a small-business team platform I previously worked on, where organizational accounts share credentials (Stripe, SendGrid, social-media admins, etc.) across their team. The Stellen implementation was a single self-managed AES-256-GCM key held in a process environment variable, with ciphertext stored in Postgres and a custom application-level access log. Team Vault is the same pattern rebuilt on AWS primitives: KMS-rotated keys, CloudTrail audit, IAM-scoped access via Cognito groups. Several friction observations later in this document are sharpened by the comparison.
...
**1. IAM Access Analyzer's `validate-policy` does not flag `Principal: "*"` in trust policies — the most-impactful IAM mistake.** *(severity: high)* Even with the correct `--validate-policy-resource-type AWS::IAM::AssumeRolePolicyDocument` flag set, `validate-policy` returns zero findings on a trust policy that allows ANY AWS principal to assume the role with no `Condition` — the pattern behind the 2019 Capital One breach. AA appears to expect this to be caught by the asynchronous *external-access* analyzer, but `validate-policy` is positioned in tooling and docs as the synchronous pre-deploy check; the most-impactful mistake passes the most-reached-for tool.
...
| **D7** | AA `validate-policy` MISSES `Principal: "*"` in trust policies — the Capital One pattern | high | **§1** |
...
| 3. Public trust policy (`Principal: "*"`) | **0** | 3 (incl. **public-principal HIGH** — the Capital One pattern) | **Yes — the headline catch** |
...
- **Prior-art reference**: I previously worked on [Stellen](https://stellenapp.com/), a small-business team platform where organizational accounts share internal credentials (Stripe, SendGrid, social-media admins, etc.) across their team via a feature called GroupSecret. That implementation used a single self-managed AES-256-GCM key held in a process environment variable, with ciphertext stored in Postgres and a custom application-level access log. The contrast between that stack and the AWS-native primitives used here (KMS-rotated keys, CloudTrail-captured key use, DynamoDB-backed audit, IAM-scoped access via Cognito groups) sharpened several of the friction observations above — particularly entry #2 (KMS double grant), entry #6 (`cfn-exec-role` overprovisioning), and the AI advisor experiment.
